...
smp_size <- floor(0.66 * nrow(catcher.data))

## set the seed to make your partition reproducible
set.seed(123)

#get the random indices for training rows
train_ind <- sample(seq_len(nrow(catcher.data)), size = smp_size)

#set training and test sets
train <- catcher.data[train_ind,]

test = catcher.data[-train_ind,]
...
save.image("~/ML Group Project NFL/WR.TE Models.RData")
