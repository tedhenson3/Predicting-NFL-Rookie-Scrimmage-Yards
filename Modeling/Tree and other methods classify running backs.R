...
smp_size <- floor(0.66 * nrow(rb.data.classifier))

## set the seed to make your partition reproducible
set.seed(123)

#get the random indices for training rows
train_ind <- sample(seq_len(nrow(rb.data.classifier)), size = smp_size)

#set training and test sets
train <- rb.data.classifier[train_ind,]

test = rb.data.classifier[-train_ind,]
...
save.image("~/ML Group Project NFL/RB class Models.RData")
